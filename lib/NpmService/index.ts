class NpmService {
  ...
  async getPackageSlugs(): Promise<string[]> {
    const response = await fetch('https://api.npms.io/v2/search?q=author:alexmacarthur');
  ...
  async getTotalDownloads() {
    ...
    for (const slug of await this.getPackageSlugs()) {
      const response = await fetch(`https://api.npmjs.org/downloads/range/last-year/${slug}`);
